...
// Auth 管理用户凭证：日历操作一律以用户身份执行，这里负责用户 token 的获取/刷新/持久化。
// 只读查询（会议室、通讯录、忙闲）走 SDK 自动管理的 tenant_access_token。
type Auth struct {
...
func (a *Auth) UserAccessToken(ctx context.Context) string {
	...
	if !IsRefreshTokenValid(stored, now) {
		a.Log.Warn("用户凭证已过期(refresh_token 失效)，需重新运行 login")
		return ""
	}
	...
	if err != nil {
		a.Log.Error(fmt.Sprintf("刷新用户 access_token 失败: %v", err))
		return ""
	}
...
// CalendarAuthOptions 日历操作的鉴权 option：只支持用户身份，无有效用户凭证时报错。
func (a *Auth) CalendarAuthOptions(ctx context.Context) ([]larkcore.RequestOptionFunc, error) {
	userToken := a.UserAccessToken(ctx)
	if userToken == "" {
		return nil, fmt.Errorf("无有效用户凭证，请先运行 room login")
	}
	return []larkcore.RequestOptionFunc{larkcore.WithUserAccessToken(userToken)}, nil
}
...
func (a *Auth) warnIfAuthNearExpiry(token *StoredUserToken, now time.Time) {
	...
	if remainDays > 0 {
		...
		a.Log.Warn("用户授权已到期(auth_exp)，请重新运行 login 重新授权")
	}
}
