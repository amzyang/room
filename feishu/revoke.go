...
// 返回远端撤销是否成功（logout 的 remote_revoked 上报用）。
func RevokeStoredTokensBestEffort(ctx context.Context, store UserTokenStore, revoker TokenRevoker, log *slog.Logger) bool {
	token := store.Read()
	if token == nil {
		return false
	}
